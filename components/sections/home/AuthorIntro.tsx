...
import { Section, Container, SectionHead, Button, FloatingDoodle } from '@/components/ui';
...
export function AuthorIntro() {
  ...
  // Get first paragraph of bio
  const firstParagraph = author.bio.split('\n\n')[0] || author.bio;

  return (
    <Section bg="secondary-soft" id="author-intro">
      <FloatingDoodle shape="blob" color="text-secondary-200" animation="float" className="absolute -top-10 -right-10 w-64 h-64 opacity-50" />
      <FloatingDoodle shape="arrow" color="text-secondary-300" animation="wiggle" className="absolute bottom-10 left-10 w-20 h-20 -rotate-12" />
      <Container>
        <motion.div
          ref={ref}
        ...
        >
          ...
          <motion.div variants={itemVariants} className="w-full md:w-1/3 flex justify-center">
            <div className="relative w-56 h-64 md:w-64 md:h-72 bg-white p-4 pb-12 shadow-[8px_8px_0px_rgba(15,23,42,1)] border-4 border-slate-900 -rotate-2 hover:rotate-1 transition-transform group">
              {/* Tape */}
              <div className="absolute -top-4 left-1/2 -translate-x-1/2 w-16 h-8 bg-secondary-400 opacity-90 rotate-3 z-20" style={{ clipPath: 'polygon(5% 0, 95% 5%, 100% 95%, 0 100%)' }}></div>
              ...
              <div className="absolute bottom-2 left-0 w-full text-center">
                 <span className="font-handwriting text-3xl text-slate-800">Hi, I&apos;m Rich!</span>
              </div>
            </div>
          </motion.div>
          ...
          <div className="w-full md:w-2/3 flex flex-col items-center md:items-start">
            ...
            <motion.p variants={itemVariants} className="font-body text-slate-600 text-base leading-relaxed mt-4 text-center md:text-left">
              {firstParagraph}
            </motion.p>
            
            <motion.div variants={itemVariants} className="flex flex-col gap-3 mt-6 w-full">
